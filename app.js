(() => {
  ...
  function renderMonsters() {
    const q=$('#monsterSearch').value.trim().toLowerCase(),cr=$('#monsterCrFilter').value,size=$('#monsterSizeFilter').value,type=$('#monsterTypeFilter').value,alignment=$('#monsterAlignmentFilter').value;
    const matches=(value,filter)=>!filter||String(value||'').trim().toLowerCase()===filter.toLowerCase();
    const filtered=state.monsters.filter(m=>(!q||`${m.name} ${m.type} ${m.challenge_rating}`.toLowerCase().includes(q))&&matches(m.challenge_rating,cr)&&matches(m.size,size)&&matches(m.type,type)&&matches(m.alignment,alignment));
    ...
    const valuesFor=field=>[...new Map(state.monsters.map(m=>String(m[field]||'').trim()).filter(Boolean).map(value=>[value.toLowerCase(),value])).values()];
    const populate=(selector,label,values)=>{const select=$(selector),current=select.value;select.innerHTML=`<option value="">All ${label}</option>`+values.map(value=>`<option value="${esc(value)}">${esc(value)}</option>`).join('');select.value=values.some(value=>value.toLowerCase()===current.toLowerCase())?values.find(value=>value.toLowerCase()===current.toLowerCase()):'';};
    populate('#monsterCrFilter','challenge ratings',valuesFor('challenge_rating').sort((a,b)=>parseFloat(a)-parseFloat(b)));
    populate('#monsterSizeFilter','sizes',valuesFor('size').sort((a,b)=>a.localeCompare(b)));
    populate('#monsterTypeFilter','creature types',valuesFor('type').sort((a,b)=>a.localeCompare(b)));
    populate('#monsterAlignmentFilter','alignments',valuesFor('alignment').sort((a,b)=>a.localeCompare(b)));
  }
  ...
  async function confirmFiveToolsImport() {
    ...
    try{const worker=async()=>{while(cursor<selected.length){const index=cursor++,item=selected[index];if(progress)progress.textContent=`Downloading ${complete+1} of ${selected.length}: ${item.code}…`;const response=await fetch(item.url);if(!response.ok)throw new Error(`${item.code} failed (${response.status}).`);payloads[index]=await response.json();complete++;}};await Promise.all(Array.from({length:Math.min(4,selected.length)},worker));if(progress)progress.textContent='Converting and saving…';const result=applyFiveToolsRecords(kind,payloads,selected.map(item=>item.code));closeDialog();toast(`${result.total} ${kind} imported · ${result.refreshed} refreshed${result.duplicatesRemoved?` · ${result.duplicatesRemoved} duplicate${result.duplicatesRemoved===1?'':'s'} removed`:''}`);
  ...
  function importSpells(file) { const reader=new FileReader();reader.onload=()=>{try{const raw=JSON.parse(reader.result);if(raw&&Array.isArray(raw.spell)){const sources=[...new Set(raw.spell.map(item=>item.source).filter(Boolean))],result=applyFiveToolsRecords('spells',[raw],sources);return toast(`${result.total} 5etools spells imported${result.duplicatesRemoved?` · ${result.duplicatesRemoved} duplicate${result.duplicatesRemoved===1?'':'s'} removed`:''}`);}const arr=Array.isArray(raw)?raw:Array.isArray(raw.results)?raw.results:[raw];let added=0,updated=0;arr.forEach(item=>{const s=normaliseSpell(markCustom(item)),idx=state.spells.findIndex(x=>x.import_source!=='5etools'&&x.name.toLowerCase()===s.name.toLowerCase());if(idx>=0){s.id=state.spells[idx].id;state.spells[idx]=s;updated++;}else{state.spells.push(s);added++;}});state.spells.sort((a,b)=>a.name.localeCompare(b.name));save();renderSpells();toast(`${added} added · ${updated} updated`);}catch(error){console.error(error);toast('That file is not valid spell JSON');}};reader.readAsText(file); }

  function renderParties() {
    const search=$('#partySearch'),q=search?search.value.trim().toLowerCase():'',parties=state.parties.filter(p=>!q||[p.name,...p.members.flatMap(member=>[member.name,member.player,member.className])].filter(Boolean).join(' ').toLowerCase().includes(q));
    $('#partyGrid').innerHTML=parties.length?parties.map(p=>`<article class="card party-card"><button class="party-delete-button" data-delete-party="${p.id}" aria-label="Delete ${esc(p.name)}" title="Delete ${esc(p.name)}">×</button><p class="eyebrow">${p.members.length} MEMBERS</p><h2>${esc(p.name)}</h2><p class="meta">Ready player roster</p><div class="roster-preview">${p.members.map(m=>`<span>${esc(m.name)} · AC ${m.ac}</span>`).join('')}</div><div class="card-actions"><button class="button ghost" data-edit-party="${p.id}">Edit</button><button class="button primary" data-add-party="${p.id}">Add to combat</button></div></article>`).join(''):state.parties.length?'<div class="empty-collection"><h2>No matching parties</h2><p>Try a party, character, player, or class name.</p></div>':'<div class="empty-collection"><h2>No parties yet</h2><p>Create your regular player roster once, then reuse it in every encounter.</p></div>';
  }
